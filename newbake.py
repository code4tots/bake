...
def lex(string):
	...
	while i < len(string):
		for token_type, regex in token_types.items():
			...
			raise Exception('unrecognized token: ' + err_regex.match(string,i).group())
...
def parse_primary_expression(token_stream):
	...
	if type_ in ('int', 'float', 'str', 'name') or token in ('(','\\'):
		...
		if type_ == 'name':
			...
			return '(new Function([&]('+','.join('Pointer '+argument for argument in arguments)+')'+body+'))'
...
def parse_statement(token_stream):
	...
	for parser in (parse_empty_statement,parse_expression_statement,parse_block_statement,parse_declaration_statement):
		result = parser(token_stream)
		if result:
			return result
...
def parse(token_stream):
	...
	raise Exception('invalid statement on line ' + str(token.line_number) + '\n' + token.line)


if __name__ == '__main__':
	try:
		
		ts = TokenStream(lex(r"""
		c         # primary expressions
		3
		"hello"

		(forth)   # parenthetical expressions


		(a+b)*c   # (binary) operator precedence
		a+b*c
		...
		f(a,b)    # test out function calls

		(                   # lexical analysis (ignore newlines when inside parenthesis)
			hello
		) + 12

		f = \ a b c {       # function literals
			hello(there)
			a+b
		}
